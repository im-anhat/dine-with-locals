...
export const getNearbyListings = async (req, res) => {
    try {
        ...
        const nearbyListings = listings.filter((listing) => {
            ...
            const calculatedDistance = calculateDistance(parseFloat(lat), parseFloat(lng), location.coordinates.lat, location.coordinates.lng);
...
export const updateListing = async (req, res) => {
    try {
        ...
        const updatedListing = await Listing.findByIdAndUpdate(listingId, updateData, { new: true, runValidators: true })
            .populate('userId', 'userName firstName lastName avatar')
            .populate('locationId');
        if (!updatedListing) {
            res.status(404).json({
