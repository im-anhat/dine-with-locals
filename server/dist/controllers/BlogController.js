...
export const createBlog = async (req, res) => {
    try {
        ...
        const populatedBlog = await Blog.findById(newBlog._id).populate('userId', 'userName firstName lastName avatar');
        if (!populatedBlog) {
...
export const updateBlog = async (req, res) => {
    try {
        ...
        if (!blogTitle && !blogContent && !photos && listingId === undefined) {
            res.status(400).json({
                error: 'At least one of blogTitle, blogContent, photos, or listingId must be provided',
            });
            return;
        }
        // Validate listingId if provided
        ...
        if (listingId !== undefined)
            updateData.listingId = listingId || null;
        // Update the blog
