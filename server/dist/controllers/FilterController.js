...
export const fetchRequestDocuments = async (req, res) => {
    const { startDate, endDate, dietaryRestriction, locationType, numGuests, category, city, } = req.body;
    ...
    //Validation
    if (numGuests)
        matchConditions.numGuests = numGuests[0];
    if (locationType)
...
export const fetchListingDocuments = async (req, res) => {
    const { startDate, endDate, dietaryRestriction, locationType, numGuests, category, city, } = req.body;
    let page = parseInt(req.query.p || '1');
    let cardPerPage = 10;
    const matchConditions = {};
