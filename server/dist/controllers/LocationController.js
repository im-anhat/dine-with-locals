...
export const createNewLocation = async (req, res) => {
    ...
    const response = await axios.get(`https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(addressString)}&key=${googleMapsApiKey}`);
    try {
 ...
 */
export const updateLocationCoordinates = async (req, res) => {
    try {
        const { locationId } = req.params;
        ...
        const location = await Location.findByIdAndUpdate(locationId, { coordinates }, { new: true, runValidators: true });
        if (!location) {
            res.status(404).json({
                message: 'Location not found',
            });
...
export const getNearbyLocations = async (req, res) => {
    try {
        ...
        const nearbyLocations = locations.filter((location) => {
            ...
            const dLat = toRadians(location.coordinates.lat - parseFloat(lat));
            const dLon = toRadians(location.coordinates.lng - parseFloat(lng));
            const a = Math.sin(dLat / 2) * Math.sin(dLat / 2) +
                Math.cos(toRadians(parseFloat(lat))) *
                    Math.cos(toRadians(location.coordinates.lat)) *
                    Math.sin(dLon / 2) *
                    Math.sin(dLon / 2);
            const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
            const calculatedDistance = R * c; // Distance in kilometers
            const distanceParam = typeof distance === 'number'
                ? distance
                : parseFloat(distance);
