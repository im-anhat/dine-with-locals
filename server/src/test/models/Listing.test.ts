...
describe('Listing Model Tests', () => {
  ...
  let testLocationId: Types.ObjectId;
  let validListingData: any; // Will be defined after setup
  ...
  beforeAll(async () => {
    ...
    validListingData = {
      ...
      description:
        'Join me for an authentic Italian dinner in my cozy home kitchen',
  ...
  describe('Listing Creation', () => {
    it('should create a new listing with valid data', async () => {
      ...
      expect(savedListing.locationId.toString()).toBe(
        testLocationId.toString(),
      );
    ...
    it('should handle all category types', async () => {
      const categories = ['dining', 'travel', 'event'] as const;
  ...
  describe('Listing Updates', () => {
    it('should update listing status', async () => {
      ...
      expect(updatedListing.updatedAt.getTime()).toBeGreaterThan(
        updatedListing.createdAt.getTime(),
      );
    });
