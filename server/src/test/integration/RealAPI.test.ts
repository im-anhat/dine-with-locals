...
describe('API Integration Tests', () => {
  ...
  describe('Authentication Endpoints', () => {
    it('should have auth routes available', async () => {
      ...
      const signupResponse = (await request(app)
        ...
        .send({})) as any; // Invalid data, but should get a response
  ...
  describe('API Route Structure', () => {
    it('should have user routes', async () => {
      const response = (await request(app).get('/api/users')) as any;
    ...
    it('should have listing routes', async () => {
      const response = (await request(app).get('/api/listing')) as any;

      expect([200, 401, 403, 404, 500]).toContain(response.status);
    });

    it('should have blog routes', async () => {
      const response = (await request(app).get('/api/blogs')) as any;
  ...
  describe('CORS and Headers', () => {
    it('should handle preflight requests', async () => {
      const response = (await request(app).options('/api/auth/login')) as any;
