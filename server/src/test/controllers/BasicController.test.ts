...
describe('Basic Controller Tests', () => {
  ...
  describe('Echo Endpoint', () => {
    ...
    it('should handle large request bodies', async () => {
      ...
      expect(response.body.received.nested.deep.object.with.many.levels).toBe(
        'value',
      );
    });
  });
  ...
  describe('Response Timing and Performance', () => {
    ...
    it('should handle concurrent requests', async () => {
      const promises = Array.from({ length: 10 }, (_, i) =>
        request(app).post('/echo').send({ requestId: i }).expect(200),
      );
  ...
  describe('Edge Cases and Error Recovery', () => {
    ...
    it('should handle deeply nested objects', async () => {
      ...
      expect(response.body.received.level1.level2.level3.level4.value).toBe(
        'deep',
      );
    });
